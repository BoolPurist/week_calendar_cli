...
static RESOURCE_FOLDER: LazyLock<PathBuf> = LazyLock::new(|| {
...
static TEMPLATE: LazyLock<TemplateDataForReadme> = LazyLock::new(|| {
    let path = RESOURCE_FOLDER.join("TEMPLATE_DATA.toml");
...
pub fn build(shell: Shell, project_path: &Path) -> AppResult {
    ...
    let about_main = shell.read_file(RESOURCE_FOLDER.join(template_data.about()))?;
    ...
    let readme_content = shell
        .read_file(RESOURCE_FOLDER.join(template_data.readme_template()))?
